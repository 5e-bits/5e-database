...
function fill_in_features(datatype_string, class_name) {
  ...
  let fill_in = (err, data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      if (
        data[i].class.name ===
        class_name.charAt(0).toUpperCase() + class_name.slice(1).toLowerCase()
      ) {
        ...
        getLevelFeaturesFromAPI(class_name, level, filled => {
          ...
          if (newData.length == 20) {
            let output_filename =
              "./upload-5e-SRD-" + datatype_string + ".json";
            fs.writeFile(output_filename, JSON.stringify(data), err => {
              if (err) throw err;
              console.log("Success");
              console.log(
                "mongoimport -h ds133158.mlab.com:33158 -d 5e-srd-api -c " +
                  datatype_string +
                  " -u admin -p password --file upload-5e-SRD-" +
                  datatype_string +
                  ".json --jsonArray"
              );
            });
